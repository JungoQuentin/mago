...
impl MagoWorkspace {
    pub async fn initialize(interner: &ThreadedInterner, root: PathBuf) -> Result<Self, Error> {
        ...
        let mut project_builder = ProjectBuilder::from_reflection(
            interner.clone(),
            reflect_non_user_sources(interner, configuration.php_version, &source_manager).await?,
        );
        ...
        let project = project_builder.build(true);

        Ok(MagoWorkspace { configuration, source_manager, project })
    }

    pub async fn get_workspace_diagnostic_report(&self) -> Result<WorkspaceDiagnosticReport, Error> {
        let mut hashmap = HashMap::default();
        for semantics in self.project.modules.iter() {
    ...
    pub async fn get_document_diagnostic(
        ...
        let semantics = self.project.modules.iter().find(|semantics| {
